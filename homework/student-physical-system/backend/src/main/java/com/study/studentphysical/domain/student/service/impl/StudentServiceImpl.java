...
import com.study.studentphysical.domain.student.dto.FailingWarning;
...
public class StudentServiceImpl implements StudentService {
    ...
    @Override
    public FailingWarning getFailingWarning() {
        return studentMapper.selectFailingWarning();
    }
    ...
    private void normalizeQuery(StudentQuery query) {
        ...
        if (query.getGender() != null) {
            query.setGender(query.getGender().trim());
        }
        if (query.getClassName() != null) {
            query.setClassName(query.getClassName().trim());
        }
    }
