...
def sample_triangle_gray(img, p1, p2, p3):
    cx, cy = triangle_centroid(p1, p2, p3)
    samples = (
        (int(cx), int(cy)),
        (int((p1[0] + cx) / 2), int((p1[1] + cy) / 2)),
        (int((p2[0] + cx) / 2), int((p2[1] + cy) / 2)),
        (int((p3[0] + cx) / 2), int((p3[1] + cy) / 2)),
    )
    total = 0
    count = 0

    for x, y in samples:
        if inside_image((x, y), img.width(), img.height()):
...
def score_triangle(img, p1, p2, p3, img_center):
    ...
    if sample_triangle_gray(img, p1, p2, p3) > TRIANGLE_SAMPLE_GRAY_MAX:
        return None
...
def find_best_triangles(img, lines, img_width, img_height):
    pair_points = {}
    ...
    for i in range(line_count):
        for j in range(i + 1, line_count):
            ...
            for k in range(j + 1, line_count):
                ...
                score = score_triangle(img, p1, p2, p3, img_center)
                if score is None:
                    continue
...
while True:
    ...
    triangles = find_best_triangles(img, lines, img.width(), img.height())
